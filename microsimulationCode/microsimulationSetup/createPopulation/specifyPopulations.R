  ...
  # -----
    ...
    # characteristics
        ...
        MSSample[seedDT, seedGroup := i.seedGroup,
                  on = .(onsetAge, currentAge, femaleGender, onsetEDSS, startingDMT = dmtID,
                         RRMSType)]

    # Split into individual populations on seedGroup and parameter set ID
    
        MSSampleSplits <- split(MSSample, by = c('seedGroup', 
                                                 'parameterSetID'))
    ...
    # If fixed (single input parameter) variations are specified expand
    # with the variations array specified in the run controls input excel
    
     if(variations_Switch == 1){
       
       MSSampleSplits <- as.data.table(expand_grid(MSSampleSplits, variationArray))
      
       MSSampleSplits[, runDescription :=
                        paste0(runDescription, '_', varyingVar, '_vary_',
                               get(varyingVar))]
       
     } else {
       
       MSSampleSplits[, runDescription := runDescription]
