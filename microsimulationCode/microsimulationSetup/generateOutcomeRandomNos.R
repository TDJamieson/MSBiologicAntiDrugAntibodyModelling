...
# ============================================================================ #  
  ...
  # table to include a random number to determine outcome for every cycle     --
      ...
      # Outcomes needing single value only
        ...
        ADAOutcomeRandomNumberFields <- c(paste0('dmt_', 
                                                 ADARiskTable[, dmtID], 
                                                 '_OutcomeRandNo'))
  ...
  if(parallelise == 1){
      ...
      # Create a table for the ADA outcomes for simplicity in the 
      # microsimulation process
      
         dmtADAOutcomeFields <- colnames(outcomeRandNos)[
           grepl('dmt_', colnames(outcomeRandNos), perl = TRUE)] 
         
         dmtADAoutcomeRandNos <- outcomeRandNos[, c(..dmtADAOutcomeFields, 
                                                    'combinedID')] 
         
         outcomeRandNos[, c(dmtADAOutcomeFields) := NULL]
         
         dmtADAoutcomeRandNos <- melt(dmtADAoutcomeRandNos, 
                                      id.vars = c('combinedID'), 
                                      variable.name = 'dmtID',
                                      value.name = 'ADAOutcomeRand')
         
         dmtADAoutcomeRandNos[, dmtID := as.numeric(gsub("\\D", "", dmtID, 
                                                             perl = TRUE))]
