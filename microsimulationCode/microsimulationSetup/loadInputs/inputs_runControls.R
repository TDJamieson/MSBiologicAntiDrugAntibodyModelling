...
# ----------
  ...
  # Relapse rate modifier                                                     --
    
    ARRMultiplier_Value <- as.numeric(read.xlsx(modelControls, 
                                           namedRegion = "ARRMultiplier_Value", 
                                           colNames = FALSE))
...
# ----
    ...
    nCycles <- (100 - preSpecifiedPopulation[which.min(MinAge), MinAge] + 1)
...
# ---------
    ...
    # Identify if cladribine and alemtuzumab are included
    alemtuzumabIncluded = 0
    
    if(!is_empty(includedDMTList[grepl('alemtuzumab', Name,
                                           ignore.case = TRUE, perl = TRUE), dmtID])){
      alemtuzumabIncluded = 1
    
    }
    
    
    cladribineIncluded = 0
    
    if(!is_empty(includedDMTList[grepl('cladribine', Name,
                                           ignore.case = TRUE, perl = TRUE), dmtID]) > 0){
      cladribineIncluded = 1
    
    }
    
    
    # alemtuzumab
    if(alemtuzumabIncluded == 1){
    ...
    if(alemtuzumabIncluded == 1){
      ...
      cladribineIncluded <-  cladribineID <- includedDMTList[grepl('cladribine 3.5mg', Name,
                                          ignore.case = TRUE, perl = TRUE), dmtID]
    } else {
...
# --------
  ...
  if(startingTreatmentLine > 1){
    ...
    switchingTables <- switchingTables[!grepl("FirstLine$", names(switchingTables))]
...
# -----------
    ...
    if(variations_Switch == 1){
      ...
      variationArray <- read.xlsx(modelControls, namedRegion = "variationArray",
                            rowNames = FALSE, colNames = FALSE)
      
      varyingVar <- as.character(
                      read.xlsx(modelControls, namedRegion = "varyingVar",
                                rowNames = FALSE, colNames = FALSE)
                      )
      
      setDT(variationArray)
      
      setnames(variationArray, 'X1', varyingVar)
