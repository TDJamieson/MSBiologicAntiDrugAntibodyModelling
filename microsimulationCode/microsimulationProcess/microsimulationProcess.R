...
#                                                                              #
# All individuals, RRMS or SPMS may die in each cycle. All individuals will    # 
# also age by one year in each cycle, incremented at the end of the cycle.     #
#                                                                              #
#-                                                                             #
#                                                                              #
...
#------------------------------------------------------------------------------#
  ...
  # -------
    ...
    # outcomes and values that influence processing within cycles             --
      ...
      # Field to follow treatment sequence, when intolerance arises,          --
      # a counter for duration that the current DMT has been used,            --
      # and markers for having PML, Graves, and Hashimoto's, and the cycle    --
      # to test in for them                                                   --
  ...
  # -------    
    ...
    # processing to be derived from                                           --
      ...
      # Mark initial disease-modifying therapy and time on DMT as 1           --
        ...
        workingSample[!is.na(DMT_Cycle0) & DMT_Cycle0 != withdrawnID, 
                      timeOnAnyDMT := 1]
        
        
      # If DMT effects are off assign everybody placebo                       --
        
        if(DMTEffect_Switch == 0){
          
          workingSample[, DMT_Cycle0 := 0]
  ...
  # --------
    ...
    # Assign new empty columns for this cycle                                 --
        ...
        workingSample[, c(currentCycleRRMSType) := NA]
  ...
  # ----
   ...
   # Remove last cycle's - suppress warnings since in cycle 0 these won't exist
  ...
  # Assign relapses in RRMS group then determine RRMS to SPMS transitions ======
    ...
    # Test that there are still members of whole sample that are in RRMS      --
      ...
      if (nrow(workingSample[RRMSCohort == 1L & is.na(traceComplete)]) > 0) {
         ...
         # Create subsample of only those in RRMS                             --
 ...
 # =========================================================================== #
  ...
  # Identify current cycle EDSS state ===========================================
   ...
   # a value for any given person. Na.rm is therefore necessary               --
    ...
    # Otherwise, just take the SPMS EDSS column value, if this has been       --
 ...
 # Determine switching  ========================================================
    ...
    # DMT switching is turned on in spreadsheet                               --
      ...
      # for the next cycle to those already withdrawn 
       ...
       # If DMT effects are off assign everybody placebo again
        
        if(DMTEffect_Switch == 0){
          
          workingSample[, c(nextCycleDMTCol) := 0]
            ...
            outcomesListComplete[[cycle]] <- as.data.table(
                                    workingSample[traceComplete == 1L, 
                                                  c('combinedID', 'interventionID', 'seedGroup', 
                                                    ...
                                                    'DMTSequence', 'DMTByCycle', 'DMTIntoleranceCycles', 'activityAtSwitch', 'timeOnAnyDMT',
                                                    'DMTWithdrawalReason', 'interventionCohort', 'ATECycle',
                                                    'PML', 'graves', 'hashimotos', ..alemtuzumabFields, ..ADAFields,
                                                    'runDescription')]
                                  )
 ...
 # Produce all final outcomes =================================================
     ...
     # Remove unneeded tables to make space
      ...
      # Return list of outcome lists                                          --
          ...
          return(microsimulationOutcome = workingSample)
