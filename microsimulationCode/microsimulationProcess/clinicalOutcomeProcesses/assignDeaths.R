...
assignDeaths <- function (population = workingSample, cycleNum = cycle) {
  ...
  # Merge EDSS-specific multiplier                                            --
      ...
      population[, currentCycleEDSS := get(paste0('EDSS_Cycle', cycleNum))]
      population[mortalityEDSSMultiplierDT[, .(EDSS, Multiplier, parameterSetID)],
                 multiplier := i.Multiplier,
                 on = .(currentCycleEDSS = EDSS, parameterSetID)]
      population[, currentCycleEDSS := NULL]
  ...
  # Mark those that have just died as traceComplete so they do not continue  --
  # being simulated                                                           --
