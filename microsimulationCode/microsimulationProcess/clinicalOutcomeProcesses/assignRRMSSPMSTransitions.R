...
assignRRMSSPMSTransitions <- function(RRMSPopulation = RRMSSubset,
 ...
 # because data.table returns a vector rather than a list)                    --
  ...
  # Those who were EDSS 9 coming in need to be dealt with separately due to   --
  # the inability to progress beyond 9 - these do not transition if they end  --
  # up in the EDSS 8 bucket & do transition if in EDSS 9 
  
    RRMSPopulation[lastCycleEDSS == 9 &
                   is.na(get(currentCycleEDSSCol)), c(currentCycleEDSSCol) := 9]
  
    RRMSPopulation[lastCycleEDSS == 9 &
                   get(currentCycleEDSSCol) == 8, c(currentCycleEDSSCol) := NA]
  ...
  # Withdraw DMTs from those that have transitioned to SPMS                   --
      ...
      RRMSPopulation[SPMSCohort == 1L & is.na(DMTWithdrawalReason), 
                     DMTWithdrawalReason := 'SPMS Transition']
